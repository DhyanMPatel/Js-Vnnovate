...
// validitycheck(country, addrErr);
// validitycheck(state, addrErr);
// validitycheck(city, addrErr);

function validitycheck(elem, errElem) {
  elem.addEventListener("input", () => {
    if (elem.validity.patternMismatch) {
      errElem.innerHTML = `Enter Valid ${elem.name}`;
    ...
    if (elem.validity.valueMissing) {
      errElem.innerHTML = `Enter Your ${elem.name}`;
      elem.style.borderColor = "red";
    }
    if (elem.validity.tooShort) {
      errElem.innerHTML = `${elem.name} is Too Short`;
    }
  });
}
